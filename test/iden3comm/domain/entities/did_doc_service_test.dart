import 'dart:convert';

import 'package:flutter_test/flutter_test.dart';
import 'package:polygonid_flutter_sdk/iden3comm/domain/entities/common/did_doc/did_document_service.dart';

String data = '''
{
  "id": "did:iden3:polygon:mumbai:119tqceWdRd2F6WnAyVuFQRFjK3WUXq2LorSPyG9LJ#push",
  "type": "push-notification",
  "serviceEndpoint": "https://push.polygonid.me/api/v1",
  "metadata": {
    "devices": [
      {
        "ciphertext": "",
        "alg": "RSA-OAEP-512"
      }
    ]
  }
}
''';
var json = jsonDecode(data);

main() {
  group("DIDDocumentService", () {
    test("fromJson", () {
      final didDocumentService = DIDDocumentService.fromJson(json);
      expect(didDocumentService.id,
          "did:iden3:polygon:mumbai:119tqceWdRd2F6WnAyVuFQRFjK3WUXq2LorSPyG9LJ#push");
      expect(didDocumentService.type, "push-notification");
      expect(didDocumentService.serviceEndpoint,
          "https://push.polygonid.me/api/v1");
      expect(didDocumentService.metadata?.devices?[0].ciphertext,
          "");
      expect(didDocumentService.metadata?.devices?[0].alg, "RSA-OAEP-512");
    });

    test("toJson", () {
      var authBodyDidDocServiceResponse = DIDDocumentService.fromJson(json);
      expect(authBodyDidDocServiceResponse.toJson(), json);
    });
  });
}
