...
class Gist extends _i1.GeneratedContract {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<StateEntries> stateEntries(
    BigInt $param20, {
    _i1.BlockNum? atBlock,
  }) async {
    final function = self.abi.functions[19];
    assert(checkSignature(function, '3d8c1445'));
    ...
    return StateEntries(response);
  }

  /// The optional [atBlock] parameter can be used to view historical data. When
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> statesHistories(
    BigInt $param21,
    BigInt $param22, {
    _i1.BlockNum? atBlock,
  }) async {
    final function = self.abi.functions[20];
    assert(checkSignature(function, 'b9617362'));
    final params = [
      $param21,
      $param22,
    ];
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> verifier({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[23];
    assert(checkSignature(function, '2b7ac3f3'));
    final params = [];
    final response = await read(
      function,
      params,
...
class StateEntries {
  StateEntries(List<dynamic> response)
      : id = (response[0] as BigInt),
        timestamp = (response[1] as BigInt),
        block = (response[2] as BigInt),
        replacedBy = (response[3] as BigInt);
