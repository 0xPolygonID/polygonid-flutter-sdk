...
class JoseHeader extends JsonObject {
  ...
  /// the json string
  JoseHeader.fromBase64EncodedString(super.encodedString)
    : super.decode();
...
abstract class JoseObject {
  ...
  JoseHeader get commonProtectedHeader {
    ...
    return JoseHeader.fromJson(
      commonUnion(
        recipients.map(
          (r) => safeUnion([sharedHeader, r.protectedHeader?.toJson()]),
        ),
      ),
    );
  }
...
abstract class JoseRecipient {
  ...
  JoseRecipient({
    ...
    this.unprotectedHeader,
         ...
         safeUnion([protectedHeader?.toJson(), unprotectedHeader?.toJson()]),
       );
