...
class JsonWebKey extends JsonObject {
  ...
  EncryptionResult encrypt(
    ...
    var encrypter = _keyPair.publicKey!.createEncrypter(
  ...
  List<int> wrapKey(JsonWebKey key, {String? algorithm}) {
    ...
    var v = encrypter.encrypt(
      Uint8List.fromList(decodeBase64EncodedBytes(key['k'])),
    );
    return v.data;
  }
...
abstract class JsonWebKeySetLoader {
  ...
  static T runZoned<T>(T Function() body, {JsonWebKeySetLoader? loader}) {
    return async.runZoned(
      body,
      zoneValues: {_jsonWebKeySetLoaderToken: loader},
    );
  }
}
